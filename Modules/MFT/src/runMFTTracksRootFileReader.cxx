...
class MFTTracksRootFileReader : public o2::framework::Task
{
 public:
  ...
  void run(framework::ProcessingContext& pc)
  {
    ...
    auto numberOfTracks = tracks.size();
    if (mCurrentTrack >= numberOfTracks) {
      // if (mCurrentTrack >= 50) {
      LOG(INFO) << " MFTTracksRootFileReader::run. End of file reached";
    ...
    std::copy(tracks.begin() + mCurrentTrack, tracks.begin() + mCurrentTrack + 1, std::back_inserter(*tracksInFile));
    mCurrentTrack++;

    // fill in the message
    // LOG(INFO) << " MFTTracksRootFileReader::run. In this file there are  " << tracksInFile.size() << " tracks";
    pc.outputs().snapshot(Output{ "MFT", "TRACKSMFT", 0, Lifetime::Timeframe }, *tracksInFile);
  }

 private:
  std::unique_ptr<TFile> mFile = nullptr;
  unsigned long mCurrentTrack = 0;
